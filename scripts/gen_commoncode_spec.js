// Usage: node scripts/gen_commoncode_spec.js
// Requires: npm i -D puppeteer docx markdown-it html-docx-js
...
async function takeScreenshot(url, outPng) {
  ...
  try {
    ...
    await page.$$eval('button, a, span, div', (nodes) => {
      const t = (n) => (n.innerText || n.textContent || '').trim();
      const cand = nodes.find(n => t(n).includes('공통코드등록') && (n.tagName === 'BUTTON' || n.tagName === 'A')) || nodes.find(n => t(n).includes('공통코드등록'));
      if (cand) cand.click();
    });
    // 대상 URL로 이동했는지 최종 보장
    const targetPath = '/dashboard/mlm/basic/common-codes';
    ...
    await page.screenshot({ path: path.join(shotsDir, '05_commoncode_screen.png') });
...
function buildHtmlSpec(pngPath, outHtml) {
  ...
  <div class="card"><h2>2. 기능 설명</h2>
    <h3>버튼</h3>
    <ul><li>조회: 선택된 코드구분 데이터를 그리드에 조회 (빈칸 선택 시 전체 조회)</li>
    <li>추가: 현재 코드구분에 새 행 추가(사용여부 Y, 정렬순서=마지막+1)</li>
    <li>삭제: 선택 행 일괄 삭제(확인 팝업)</li>
    <li>저장: 현재 코드구분 데이터만 병합 저장</li></ul>
    <h3>셀렉트박스</h3><ul><li>전체, ROOT, ROOT 코드 목록(A01, B01, C01) 표시, 조회 버튼 클릭 시 조회</li></ul>
    <h3>그리드</h3><ul><li>선택(체크), 코드구분(읽기전용), 코드(최대 20자), 코드명, 정렬순서(0~999, 3자리), 사용여부(Y/N), 비고</li></ul>
  </div>
  <div class="card"><h2>3. 데이터구조도</h2>
    <table><thead><tr><th>필드</th><th>설명</th></tr></thead><tbody>
      <tr><td>codeDivision</td><td>코드구분 (예: "ROOT", "A01")</td></tr>
      <tr><td>code</td><td>코드</td></tr>
      <tr><td>codeName</td><td>코드명</td></tr>
      <tr><td>codeValueNum</td><td>코드값(숫자) - 옵션</td></tr>
      <tr><td>codeValueStr</td><td>코드값(문자) - 옵션</td></tr>
  ...
  <div class="card"><h2>4. UML 다이어그램</h2>
    <pre>CommonCodePage -> API Controller -> CommonCodeService -> JSON Store
...
async function buildDocx(pngPath, outDocx) {
  ...
  const doc = new Document({
    creator: 'DwFw',
    title: '공통코드 등록 화면 명세서',
    description: 'CommonCode UI Spec',
    sections: [],
  });
  ...
  doc.addSection({
    children: [
      new Paragraph({ text: '공통코드 등록 화면 명세서', heading: HeadingLevel.TITLE }),
  ...
  doc.addSection({
    children: [
      ...
      new Paragraph({ text: '- 조회: 선택된 코드구분 데이터를 그리드에 조회 (빈칸 선택 시 전체 조회)' }),
      new Paragraph({ text: '- 추가: 현재 코드구분에 새 행 추가(사용여부 Y, 정렬순서=마지막+1)' }),
      new Paragraph({ text: '- 삭제: 선택 행 일괄 삭제(확인 팝업)' }),
      new Paragraph({ text: '- 저장: 현재 코드구분 데이터만 병합 저장' }),
      new Paragraph({ text: '' }),
      new Paragraph({ text: '셀렉트박스', heading: HeadingLevel.HEADING_2 }),
      new Paragraph({ text: '- 전체, ROOT, ROOT 코드 목록(A01, B01, C01) 표시, 조회 버튼 클릭 시 조회' }),
      new Paragraph({ text: '' }),
      new Paragraph({ text: '그리드', heading: HeadingLevel.HEADING_2 }),
      new Paragraph({ text: '- 선택(체크), 코드구분(읽기전용), 코드(최대 20자), 코드명, 정렬순서(0~999, 3자리), 사용여부(Y/N), 비고' }),
    ],
  });
  ...
  const codeTable = new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [ new TableCell({ children: [ new Paragraph('codeDivision') ] }), new TableCell({ children: [ new Paragraph('코드구분 (예: "ROOT", "A01")') ] }) ] }),
      ...
      new TableRow({ children: [ new TableCell({ children: [ new Paragraph('codeValueNum') ] }), new TableCell({ children: [ new Paragraph('코드값(숫자) - 옵션') ] }) ] }),
      new TableRow({ children: [ new TableCell({ children: [ new Paragraph('codeValueStr') ] }), new TableCell({ children: [ new Paragraph('코드값(문자) - 옵션') ] }) ] }),
  ...
  doc.addSection({
    children: [
      ...
      new Paragraph({ text: 'CommonCodePage -> API Controller -> CommonCodeService -> JSON Store' }),
      new Paragraph({ text: '' }),
...
(async () => {
  const url = process.env.TARGET_URL || 'http://localhost:3000/dashboard/mlm/basic/common-codes';
  const docDir = path.resolve(process.cwd(), 'doc');
  if (!fs.existsSync(docDir)) fs.mkdirSync(docDir, { recursive: true });
  const assetsDir = path.join(docDir, 'assets');
  if (!fs.existsSync(assetsDir)) fs.mkdirSync(assetsDir, { recursive: true });
  const pngPath = path.join(assetsDir, 'commoncode_screen.png');
  const docxPath = path.join(docDir, 'CommonCode_명세서.docx');
  const pdfPath = path.join(docDir, 'CommonCode_명세서.pdf');
  const htmlPath = path.join(docDir, 'CommonCode_명세서.html');
  const mdPath = path.join(docDir, 'CommonCode_명세서.md');
  const mergedHtmlPath = path.join(docDir, 'CommonCode_명세서_full.html');
